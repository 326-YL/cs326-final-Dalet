...
<font size="4" >
  ...
  <b> Yangyang's Conclusion</b>: this is my first time to know about the server stuffs, learn much about how to set up routers, how the data being transferred between client side and server side, having struggle on transferring data bettwen two sides, but recently learn about EJS, it is useful to manipulate the data between server and client.<br>
  <b>Sam's conclusion</b>: The project overall was interesting. I couldn't figure out heroku at the start, I didn't really understand much of the purpose of other server stuff, such as the router object. Whenever the server went down, or something with git went wrong, I had to go to another group member to sort it out. By the end, I might be able to do some server stuff, but not anything more than http.createserver and get and post with express. Learning how to manipulate html for the explore page was pretty fun, trying to figure out how to take the database and get what I want on the page, and then saving previous results. Making items save to a database for users would be fun to impliment I think, if we got the login stuff to work.
</font>
